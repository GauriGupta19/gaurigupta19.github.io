...
function PostItem({ post, index }: { post: Post; index: number }) {
  ...
  return (
    <Link
      key={post.id}
      to={`/blogs/${post.id}`}
      className="group flex items-start justify-between gap-4 sm:gap-6 py-4 sm:py-5 border-b border-border hover:bg-muted/30 -mx-2 sm:-mx-3 px-2 sm:px-3 rounded-lg transition-all duration-200 animate-slide-up touch-manipulation"
      style={{ animationDelay: delay }}
    >
      <div className="flex-1 min-w-0">
        <h2 className="font-medium text-base sm:text-lg text-foreground mb-1.5 group-hover:text-muted-foreground transition-colors duration-200">
          {post.title}
        </h2>
        {post.excerpt && (
          <p className="text-sm text-muted-foreground leading-relaxed line-clamp-2 sm:line-clamp-none">{post.excerpt}</p>
        )}
        <div className="flex items-center gap-3 shrink-0 pt-2 sm:pt-3 text-xs sm:text-sm text-muted-foreground font-mono">
          {post.date}
  ...
  return (
    <div className="animate-fade-in">
      <div className="max-w-[680px] mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-24">
        ...
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8 sm:mb-12 group touch-manipulation min-h-11"
        >
        ...
        <header className="mb-8 sm:mb-12 animate-slide-up">
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight mb-2">Blogs</h1>
